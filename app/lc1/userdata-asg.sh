#!/bin/bash
sudo apt-get update
sudo apt-get install golang git awscli wget -y
...
dbhostname=`aws ssm get-parameter     --name "dbhostname"     --with-decryption --region us-east-1 --output text --query Parameter.Value`
