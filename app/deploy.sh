...
if [ "$lc" == "$lcfg1" ];then
	...
	sg=`aws ec2  describe-security-groups --filter Name=vpc-id,Values=$vpc  Name=group-name,Values=*APP*SG --region aws-region --query 'SecurityGroups[*].[GroupId]' --output text`
